...
const getBudgetStats = () => {
  ...
  return (
    ...
    <div className="flex items-center justify-between mb-8">
        <div>
            <h1 className="text-3xl font-display font-bold text-gray-900 mb-2">Budget Tracker
</h1>
            <p className="text-gray-600">
                {trip.destination} - Track your spending and stay on budget
                          </p>
        </div>
        <Button onClick={() => setShowAddModal(true)} variant="primary" icon="Plus">Add Expense
                    </Button>
    </div>
    {/* Budget Overview */}
            ...
            <Card>
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-2xl font-bold text-primary">${trip.budget.toLocaleString()}
                        </p>
                    </div>
            ...
            <Card>
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-2xl font-bold text-accent">${budgetStats.spent.toLocaleString()}
                        </p>
                        <div className="flex items-center mt-1">
                            <div className="w-full bg-gray-200 rounded-full h-2 mr-2">
                                <div
                                    className={`h-2 rounded-full transition-all duration-300 ${budgetStats.percentage > 90 ? "bg-red-500" : budgetStats.percentage > 70 ? "bg-yellow-500" : "bg-secondary"}`}
                                    style={{
            ...
            <Card>
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p
                            className={`text-2xl font-bold ${budgetStats.remaining >= 0 ? "text-green-600" : "text-red-600"}`}>${Math.abs(budgetStats.remaining).toLocaleString()}
                        </p>
                        {budgetStats.remaining < 0 && <Badge variant="error" size="sm" className="mt-1">Over Budget
                                              </Badge>}
                    </div>
                    <div
                        className={`p-3 rounded-lg ${budgetStats.remaining >= 0 ? "bg-green-100" : "bg-red-100"}`}>
                        <ApperIcon
                            name={budgetStats.remaining >= 0 ? "PiggyBank" : "AlertTriangle"}
                            className={`w-6 h-6 ${budgetStats.remaining >= 0 ? "text-green-600" : "text-red-600"}`} />
                    </div>
                </div>
            </Card>
        </motion.div>
...
<div className="flex items-center justify-between mb-6">
                ...
                {expenses.length === 0 ? <EmptyState
                    ...
                    onAction={() => setShowAddModal(true)} /> : <div className="space-y-3">
                    {expenses.slice().reverse().map((expense, index) => <motion.div
                        key={expense.Id}
                        initial={{
                            opacity: 0,
                            x: -20
                        ...
                        <div className="flex items-center space-x-3">
                            <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center ${expenseCategories.find(cat => cat.key === expense.category)?.color ? "bg-opacity-20" : "bg-gray-200"}`}
                                style={{
...
<p className="font-medium text-gray-900">{expense.description}</p>
                        ...
                        <div className="flex items-center space-x-3">
                            <span className="font-semibold text-gray-900">${expense.amount.toLocaleString()}
                            </span>
                            <Button
                                variant="ghost"
                                size="sm"
            ...
            <Card>
                ...
                <div className="space-y-3">
                    {categoryBreakdown.map(
                        category => <div key={category.key} className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                                <div
...
<div>
                            ...
                            <span className="font-semibold text-gray-900">${category.amount.toLocaleString()}
                            </span>
                        </div>
                    )}
                </div>
    ...
    {showAddModal && <>
        ...
        <motion.div
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                <h3 className="text-lg font-display font-semibold mb-4">Add New Expense
                                  </h3>
                <div className="space-y-4">
                    <div>
                        ...
                        <select
                            ...
                            {expenseCategories.map(category => <option key={category.key} value={category.key}>
                                {category.label}
                            </option>)}
                        </select>
                    </div>
                ...
                <div className="flex justify-end space-x-3 mt-6">
                    <Button variant="ghost" onClick={() => setShowAddModal(false)}>Cancel
                                        </Button>
                    <Button variant="primary" onClick={handleAddExpense}>Add Expense
                                        </Button>
                </div>
            </div>
        </motion.div>
